...
export async function POST(request: Request) {
  ...
  if (existingUserError && existingUserError.details !== "Results contain 0 rows") {
    return jsonError(500, `Failed to query user: ${existingUserError.message}`);
  }

  let teamId: number | null = null;
  if (existingUser && existingUser.team_id === null) {
    const requestedTeammates = asNullableUuidArray(obj.teammates) ?? [];
    const uniqueTeammates = Array.from(new Set(requestedTeammates.map((t) => t.trim()).filter(Boolean))).filter(
      (id) => id !== auth.user.id
    );

    const memberIds = [auth.user.id, ...uniqueTeammates];

    const { data: teamRow, error: teamError } = await supabaseAdmin
      .from("Teams")
      .insert({ teammember_ids: Array.from(new Set(memberIds)) })
      .select("team_id")
      .single();
    ...
    const createdTeamId = (teamRow as { team_id?: unknown } | null)?.team_id ?? null;
    if (createdTeamId === null || typeof createdTeamId === "undefined") {
    ...
    if (typeof createdTeamId !== "number" || !Number.isInteger(createdTeamId)) {
      return jsonError(500, "Failed to create team: team_id is not an integer.");
    }

    teamId = createdTeamId;

    // Update all team members' user rows with this team_id.
    ...
    if (assignTeamError) {
      return jsonError(500, `Failed to assign team to users: ${assignTeamError.message}`);
    }

  } else {
    teamId = (existingUser?.team_id as number | null | undefined) ?? null;
  }

  return NextResponse.json(
    { ok: true, id: applicationData.application_id, resume_path: resumePath, team_id: teamId },
